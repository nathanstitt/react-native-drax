...
import { dragTouchAction, useDragGesture } from "./hooks/useDragGesture.js";
...
const DRAX_PROP_KEYS = new Set(['renderContent', 'renderHoverContent', 'noHover', 'registration', 'onMeasure', 'measureVisual', 'parent', 'isParent', 'scrollPosition', 'longPressDelay', 'lockDragXPosition', 'lockDragYPosition', 'id',
// Callback props
...
export const DraxView = /*#__PURE__*/memo(props => {
  ...
  const measureWithHandler = useCallback(handler => {
    ...
    if (measureVisual && Platform.OS !== 'web') {
      ...
      view.measure((_vx, _vy, width, height, pageX, pageY) => {
        parentView.measure((_px, _py, _pw, _ph, parentPageX, parentPageY) => {
          ...
          finalizeMeasurement(pageX - parentPageX + parentScroll.x, pageY - parentPageY + parentScroll.y, width, height, handler);
        });
      });
      return;
    }
