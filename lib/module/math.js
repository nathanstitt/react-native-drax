 ...
 */
export function packGrid(count, numColumns, getSpan) {
  ...
  function ensureRow(row) {
    while (occupied.length <= row) {
      occupied.push(new Array(numColumns).fill(false));
    }
  }
  function isAvailable(row, col, cs, rs) {
  ...
  function markOccupied(row, col, cs, rs) {
    for (let r = row; r < row + rs; r++) {
      ensureRow(r);
      for (let c = col; c < col + cs; c++) {
        occupied[r][c] = true;
