 ...
 */
export const useViewStyles = (id, styleProps) => {
  const {
    draggedIdSV,
    receiverIdSV,
    dragPhaseSV
  ...
  const resetStyle = computeResetStyle(flatBaseStyle, flatDraggingStyle, flatDraggingWithReceiverStyle, flatDraggingWithoutReceiverStyle, flatDragReleasedStyle, flatDragInactiveStyle, flatReceiverInactiveStyle, flatReceivingStyle, flatOtherDraggingStyle, flatOtherDraggingWithReceiverStyle, flatOtherDraggingWithoutReceiverStyle);
  ...
  const animatedDragStyle = useAnimatedStyle(() => {
    ...
    if (isMe && phase === 'dragging') {
      return mergeStyles(resetStyle, flatDraggingStyle, amReceiving ? flatDraggingWithReceiverStyle : flatDraggingWithoutReceiverStyle);
    }
    ...
    if (isDragging && !isMe) {
      return mergeStyles(resetStyle, flatDragInactiveStyle, flatOtherDraggingStyle, amReceiving ? flatOtherDraggingWithReceiverStyle : flatOtherDraggingWithoutReceiverStyle);
    }
 ...
 */
function computeResetStyle(base, ...specials) {
  const reset = {};
  const baseMap = base ? Object.entries(base).reduce((acc, [k, v]) => {
    acc[k] = v;
    return acc;
  }, {}) : {};
...
//# sourceMappingURL=useViewStyles.js.map