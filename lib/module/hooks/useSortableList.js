...
export const useSortableList = options => {
  ...
  /** The rawData reference last processed by the data-change effect below. */
  const prevRawDataRef = useRef(undefined);
  ...
  useLayoutEffect(() => {
    // Reset only on a REAL data change. The effect also re-runs when other
    // deps change identity — most commonly a keyExtractor recreated inline by
    // a parent re-render (e.g. a column highlighting itself mid-drag) — and
    // running the reset then wipes live shifts, killing the cross-container
    // phantom-slot preview the moment it appears.
    if (prevRawDataRef.current === rawData) return;
   ...
   */
  const getSlotFromPosition = (contentPos, opts) => {
    const insertion = opts?.insertion ?? false;
    const pending = pendingOrderRef.current;
    ...
    if (numColumns <= 1) {
      ...
      //
      // Reorder (default): boundary is at the gap midpoint between adjacent
      // items, making forward and backward equally responsive (distance =
      // size/2 + gap/2). Using item centers (50%) would be asymmetric: the
      // hover center starts AT the forward boundary but a full item-height
      // from the backward boundary, making forward too sensitive and
      // backward too sluggish. The dragged item is IN the list, so slots
      // run 0..N-1 ("take item i's place").
      //
      // Insertion (cross-container phantom / reinjection): the dragged item
      // is NOT in the list, so there are N+1 slots — boundaries sit at item
      // centers (top half of item i → before it, bottom half → after it),
      // and a position past the last item's center returns N, the append
      // slot the reorder semantics can never reach.
      ...
      for (let i = 0; i < itemCount; i++) {
        ...
        const boundary = insertion ? cursor + size / 2 // item center
        : cursor + size + gap / 2; // midpoint of gap after item
        const pos = horizontal ? contentPos.x : contentPos.y;
        if (pos < boundary) return i;
        cursor += size + gap;
      }
      return insertion ? itemCount : itemCount - 1;
    } else if (getItemSpan) {
