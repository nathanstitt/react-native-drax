 ...
 */
function updateSpatialEntryCapabilities(spatialIndexSV, idx, draggable, receptive, monitoring, rejectOwnChildren, collisionAlgorithm) {
...
export const useSpatialIndex = () => {
  // ── SharedValues ───────────────────────────────────────────────────
  const spatialIndexSV = useSharedValue([]);
  const scrollOffsetsSV = useSharedValue([]);
  ...
  const pendingUnregistersRef = useRef(new Set());

  /** Find the spatial index of a parent view, or -1 if not found */
  const findParentSpatialIndex = parentId => {
  ...
  const registerView = payload => {
    ...
    if (existing) {
      ...
      updateSpatialEntryCapabilities(spatialIndexSV, idx, draggable, receptive, monitoring, props.rejectOwnChildren ?? false, props.collisionAlgorithm ?? 'center');
      return;
    }
    ...
    for (const [, childEntry] of registryRef.current) {
      if (childEntry.parentId === id && childEntry.id !== id) {
        ...
        spatialIndexSV.modify(entries => {
          ...
          if (childIdx >= 0 && childIdx < entries.length && entries[childIdx] && entries[childIdx].parentIndex !== spatialIndex) {
  ...
  const updateMeasurements = (id, measurements) => {
    const entry = registryRef.current.get(id);
    if (!entry) return;
  ...
  const updateScrollOffset = (id, offset) => {
    const entry = registryRef.current.get(id);
    if (!entry) return;
    const idx = entry.spatialIndex;
    scrollOffsetsSV.modify(offsets => {
  ...
  const updateViewProps = (id, props) => {
    const entry = registryRef.current.get(id);
    if (!entry) return;
    entry.props = props;
...
export function isDraggable(props) {
  return props.draggable ?? (props.dragPayload !== undefined || props.payload !== undefined || !!props.onDrag || !!props.onDragEnd || !!props.onDragEnter || !!props.onDragExit || !!props.onDragOver || !!props.onDragStart || !!props.onDragDrop);
}
function isReceptive(props) {
  return props.receptive ?? (props.receiverPayload !== undefined || props.payload !== undefined || !!props.onReceiveDragEnter || !!props.onReceiveDragExit || !!props.onReceiveDragOver || !!props.onReceiveDragDrop);
}
function isMonitoring(props) {
  return props.monitoring ?? (!!props.onMonitorDragStart || !!props.onMonitorDragEnter || !!props.onMonitorDragOver || !!props.onMonitorDragExit || !!props.onMonitorDragEnd || !!props.onMonitorDragDrop);
}
//# sourceMappingURL=useSpatialIndex.js.map